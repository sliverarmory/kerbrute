...
// Simulate command line args for parsing
func GetCommandLineArgs(data uintptr, dataLen uintptr) error {
	// Process arguments sent from the implant
	argBytes := unsafe.Slice((*byte)(unsafe.Pointer(data)), int(dataLen))
...
// Write satisfies the io.Writer interface for OutputBuffer.
// It allows OutputBuffer to be used anywhere an io.Writer is expected.
func (buff *OutputBuffer) Write(p []byte) (n int, err error) {
	buff.b.WriteString(string(p))
	return len(p), nil
}
